...
# Created: 05/01/2020
#
# This script reformats text delimited files to WHP-Exchange format using information stored within a user-entered metadata file
# The metadata file must be named "PIG_meta.csv"
# The script will generate a single file per entry row. These should then be merged to a single table in a separate step using create_layer_table.R
#
#
# file_path - path to PROF_meta.csv
# path_out - path where reformatted files are to be sent. A layer folder will be created in here, if one does not already exist
# userID - an ID code for the user creating the files
# row_start - the start row
# row_end - the end row
...
PIG_to_WHPE = function(file_path, path_out,userID = "IMASUTASKB",row_start = 1,row_end = NA){
  ...
  # just to set up pigment names and the final data frame
  ...
  pig_names = pig_names[pig_names != "Notes"]
  ...
  for(rw in row_start:row_end)
  {
    ...
    # loop through files
    for(fl in files){
            # text files - at the moment this is the only option
            if(info$file_type == "text delim"){
              ...
              names = data_vars[which(info[,..data_vars] != "")]
              ...
              if(info$TIME_analyser == "hour-minute-second"){n_headers = n_headers - 1}} # these wont be matched

              # get total number lines in the file
              f <- file( fl, open = "r" )
              ...
              while( TRUE ){
                ...
                # data variables searching for in headers
                dv = data_vars[which(info[,..data_vars] != "")]
                ...
                # if there are at least three data_vars that apear, break this is likeley the header line
                if(length(which(header_in_line == T) ) >= n_headers  & !substr(line,1,1) %in% c("!","#")){
                  # write to a file to check header line is picked and variables have been grabbed
              ...
              if(info$delim == "rect"){
                ...
                data = as.data.frame(fread(fl,stringsAsFactors = F, skip = b-1, na.strings = info$missing_value,strip.white = T, header = F ))
              }
              # get the header line
              ...
              #seabass format - adjust date and time entries
              if(info$DATE_analyser == "year-month-day"){year = data[,grep("year",colnames(data))]
                dataf$DATE_analyser = sprintf("%04d-%02d-%02d", year,data$month,data$day)
                info$DATE_analyser_format = "%Y-%m-%d"}
              if(info$TIME_analyser == "hour-minute-second"){dataf$TIME_analyser = sprintf("%02d:%02d:%02d",data$hour, data$minute, data$second)
    ...
    ### CASTNO and STNNBR
    if(info$CASTNO == info$STNNBR & !is.empty(info$STNNBR)){
    ...
    # if station numbers are not indicated in the file make up numbers based on lat/lon. if underway indicated dont replace.
    if(any(is.na(data2$STNNBR))){
      idx = is.na(data2$STNNBR)
      temp = data2[idx,] %>% mutate(STNNBR = group_indices(., LAT_analyser, LON_analyser))
      data2$STNNBR[idx] = temp$STNNBR
      rm(temp)
    }
    ...
    if(nchar(data2$DATE_analyser[which(data2$DATE_analyser != "")[1]])>12 & grepl("AADC",info$source)){data2$DATE_analyser = substr(data2$DATE_analyser,1,12)}}
    ...
    if(nrow(meta %>% filter(source == info$source, Method == info$Method, EXPOCODE == info$EXPOCODE))>1){
      new_file = paste(paste(ex,"PIG",info$source,info$Method,info$dup_id,sep = "_"),".csv",sep = "")
    }else{
    ...
    writeLines(paste("#ORIGINAL_CHLFILE/S:", paste(files,sep = ";")), fd)
    ...
    #### CTD_data ###
    # append CTD_ID, LAT, LON, DATE, TIME
    data2$CTD_IDs = NA
    # list available CTD_IDs
    if(any(!(data2$STNNBR == "U") & !is.na(data2$STNNBR) & !is.null(data2$STNNBR))){
    ctd_files = list.files(ctd_path, pattern = ".csv", full.names = F)


    data2$CTD_IDs[data2$STNNBR == "U"] = "U"
    ...
    data2_stncast = paste(data2$STNNBR,data2$CASTNO)
    ...
    data2$CTD_IDs[sdx] = paste(ex,"CTD",data2$STNNBR[sdx],data2$CASTNO[sdx],sep = "_")
    ...
    ### attempt to find matching ctd casts based on time if there are still unmatched casts
    # unatched STNNBR
    #cast numbers???
    ...
    unmatched_df = unmatched_df %>% filter(!is.na(Time), Time != "", Date != "")
    
    # first we want to match based on time, if we can
    ...
    for(ctd in CTD_info$CTD_ID){
      ...
      while( TRUE ){
        ...
        if( grepl( "STNNBR or EVENTNBR =", line ) ){
          stn = trimws(sub("STNNBR or EVENTNBR =", "", line ))
        }
        if( grepl( "CASTNO =", line ) ){
          cast = trimws(sub("CASTNO =", "", line ))
        }
        if(grepl("CTDPRS", line)){break}
      }
      close(f)

      # TO DO make sure the profile is not bad - consult quality assurance flags later
      # **** maybe gather all of the CTD information in a table for matching?
      ...
      if(any(unmatched_df$Date == date)){
        ...
        # match closest positions  **** keep doing
          res = unmatched_df_sub_p$time_m > min_t & unmatched_df_sub_t$time_m < max_t
          if(length(which(res)) > 1){res = which.min(abs(unmatched_df_sub_t$time_m - min_t))} # get minimum time difference
          if(any(res)){
            data2$CTD_IDs[data2_stncast == unmatched_df_sub_t$STNCAST[res]] = ctd
            data2$STNNBR[data2_stncast == unmatched_df_sub_t$STNCAST[res]] = stn
    ...
    writeLines(paste("NUMBER_HEADERS =",length(colnames(final_data))), fd)
