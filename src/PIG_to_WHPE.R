...
PIG_to_WHPE = function(file_path, path_out,userID = "IMASUTASKB",row_start = 1,row_end = NA){
  ...
  for(rw in row_start:row_end)
  {
    ...
    if(nchar(data2$DATE_analyser[which(data2$DATE_analyser != "")[1]])>12 & grepl("AADC",info$source)){data2$DATE_analyser = substr(data2$DATE_analyser,1,12)}} #problems with old AADC data
    ...
    for(ctd in CTD_info$CTD_ID){
      ...
      #     }
          ...
          # find closest time
          unmatched_df_t = unmatched_df %>% filter(!is.na(Time), Time != "", Date != "") %>% 
