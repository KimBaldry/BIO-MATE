...
compile_pigments <- function(path = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE/reformatted_data", ex.list = NULL){
    ...
    for(fl in files){
      ...
      file_data$Sample_ID = as.character(file_data$Sample_ID)
      file_data$DEPTH = as.character(file_data$DEPTH)
      file_data$STNNBR_analyser = as.character(file_data$STNNBR_analyser)
      file_data$BOTTLE = as.character(file_data$BOTTLE)
