...
```
 CHL and HPLC files
```{r}
data = read.csv("E:/Data_downloads/PAL-LTER/pigments/CHL/Chlorophyll (Cruise).csv", stringsAsFactors = F)
data = data %>% filter(Sampling.Device != "ice", Depth = "ice") # remove some ice samples
write.csv(data, "E:/Data_downloads/PAL-LTER/pigments/CHL/Chlorophyll (Cruise)_edited.csv", row.names = F)

split_delim_file(path = "E:/Data_downloads/PAL-LTER/pigments/CHL", file_name = "Chlorophyll (Cruise)_edited.csv", delim = ",",synonym_var_name = "studyName",station_split = F, fillcell = T)

split_delim_file(path = "E:/Data_downloads/PAL-LTER/pigments/HPLC", file_name = "High Performance Liquid Chromatography Pigments.csv", delim = ",",synonym_var_name = "studyName",station_split = F)
```

Fix 33LG20070101 HPLC depths by inserting from phaeopigment file
```{r}
# read in phaeo file
chl = read.csv("E:/Data_downloads/PAL-LTER/pigments/CHL/33LG20070101/33LG20070101_from_Chlorophyll (Cruise)_edited.csv" , stringsAsFactors = F, header = T)
# read in HPLC file
hplc = read.csv("E:/Data_downloads/PAL-LTER/pigments/HPLC/33LG20070101/old/33LG20070101_from_High Performance Liquid Chromatography Pigments.csv", stringsAsFactors = F, header = T)
...
```{r}
# read in HPLC file
hplc = read.csv("E:/Data_downloads/PAL-LTER/pigments/HPLC/33LG20131229/33LG20131229_from_High Performance Liquid Chromatography Pigments.csv", stringsAsFactors = F, header = T)
hplc[hplc == -999000] = -999
# rewrirte HPLC file
 if(!file.exists("E:/Data_downloads/PAL-LTER/pigments/HPLC/33LG20131229/adjusted")){dir.create("E:/Data_downloads/PAL-LTER/pigments/HPLC/33LG20131229/adjusted")}
write.csv(x = hplc,file = "E:/Data_downloads/PAL-LTER/pigments/HPLC/33LG20131229/adjusted/33LG20131229_from_High Performance Liquid Chromatography Pigments.csv",row.names = F)

```


fix latitudes for 320619930325 fluor file (need to be negative)
```{r}
chl = read.csv("E:/Data_downloads/PAL-LTER/pigments/CHL/320619930325/old/320619930325_from_Chlorophyll (Cruise)_edited.csv" , stringsAsFactors = F, header = T)
chl$Latitude = -chl$Latitude
write.csv(x = chl,file = "E:/Data_downloads/PAL-LTER/pigments/CHL/320619930325/320619930325_from_Chlorophyll (Cruise)_edited.csv",row.names = F)
```
...
for(sn in unique(data$Station)){
  ...
  for(tm in unique(sub_data$Date.Time)){
    sub_sub_data = data %>% filter(Station == sn, Date.Time == tm)
    sub_sub_data$cast = which(unique(sub_data$Date.Time) == tm)
    write.csv(sub_sub_data, file = file.path(path,"split",paste("35XI20090905","_",sn,"_",which(unique(sub_data$Date.Time) == tm),"_from_", "TARA_physical",".csv", sep = "")), row.names = F)
    }
...
Remove some sediment trap samples - lines 637 onwards on 06AQ20120107 HPLC data
```{r}
...
chunks = c("C1", "C2", "C3", "C4","C5")
"regression_test",
for(ch in chunks){
  #PROF_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","product_data","processing_metadata",ch),userID = "IMASUTASKB")

  PIG_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","product_data","processing_metadata",ch),userID = "IMASUTASKB")
  print(paste(ch, "has been run"))
}
...
beep(sound = 3)
```
### reformat underway files
```{r}
UWY_to_WHPE(path_out = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE/reformatted_data", file_path = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE/BIO-MATE/product_data/processing_metadata/regression_test")
```
...
```{r}
path = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE/"
PROF_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","product_data","processing_metadata","C3"),userID = "IMASUTASKB")
beep(sound = 3)
```
...
```{r}
path = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE"
PROF_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","product_data","processing_metadata","C4"),userID = "IMASUTASKB")
beep(sound = 3)
```
...
```{r}
path = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE/"
PIG_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","product_data","processing_metadata","C4"),userID = "IMASUTASKB")
```
...
```{r}
path = "C:/Users/kabaldry/OneDrive - University of Tasmania/Documents/Projects/BIO-MATE/"
PIG_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","product_data","processing_metadata","C5"),userID = "IMASUTASKB")
```
...
PROF_to_WHPE(path_out = file.path(path,"reformatted_data"), file_path = file.path(path,"BIO-MATE","processing_metadata","C6"),userID = "IMASUTASKB")
```
...
# add in troublesome AADC CTDFLUOR
```{r}
source("./data_descriptor_paper/add_extra_AADC_CTDFLUOR.R")
```
